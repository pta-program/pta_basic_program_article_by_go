...
设链表有 `n` 个结点，第 `i` 个结点的数据为 `vᵢ`，则遍历和阶乘计算的时间复杂度为 `O(n + ∑vᵢ)`；只使用固定数量的变量，额外空间复杂度为 `O(1)`。

## 常见易错点

1. 每处理一个结点后必须执行 `L = L.Next`，否则会在当前结点上死循环。
2. 阶乘的累乘初值应为 1，结点值为 0 时要保留 `0! = 1`。
3. 测试程序使用头插法建立链表，访问顺序可能与输入顺序相反；本题求和与顺序无关，但遍历仍应从头结点开始。
4. 链表为空时总和应保持为 0，不能直接访问空指针的 `Data` 字段。
