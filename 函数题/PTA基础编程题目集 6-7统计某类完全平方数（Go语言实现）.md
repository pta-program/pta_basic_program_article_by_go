...
设 N 有 `d` 位数字，开平方和拆位的处理成本为 `O(d)`，两两比较数字的成本为 `O(d²)`；因此总时间复杂度为 `O(d²)`，额外空间复杂度为 `O(d)`。

## 常见易错点

1. 负数不可能是完全平方数，应先返回 0，不能直接对负数开平方。
2. 仅取整数平方根还不够，必须再次检查 `root * root == N`。
3. “至少有两位相同”要求比较不同位置的数字，不能只检查相邻数字。
4. 单位数没有两位可供比较，即使它是完全平方数也应返回 0。
