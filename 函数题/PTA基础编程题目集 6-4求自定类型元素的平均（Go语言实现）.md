...
遍历 N 个元素并进行一次累加，时间复杂度为 `O(N)`；只使用总和与下标等固定变量，额外空间复杂度为 `O(1)`。

## 常见易错点

1. 平均值应为总和除以 N，不能忘记除法或误除以数组容量。
2. 除法前要将 N 转为浮点数，避免发生整数除法。
3. `N` 为正整数，不能在 N 为 0 时直接求平均；若自行扩展输入范围，应先处理空数组。
4. 返回值必须转换为 `ElementType`，以符合函数接口定义。
