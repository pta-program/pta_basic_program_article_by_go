...
需要依次处理 `a[1]` 到 `a[n]`，时间复杂度为 `O(n)`；只保存当前幂次和累加结果，额外空间复杂度为 `O(1)`。

## 常见易错点

1. `sum` 必须从 `a[0]` 开始，`m` 初始为 `1.0` 才表示 `x^0`。
2. 每轮应先执行 `m *= x` 再使用 `a[i] * m`，否则幂次会错一位。
3. 系数 `a[i]` 的下标与幂次 `x^i` 必须对应，不能按输入顺序之外的方向读取。
4. 计算过程中使用浮点数，避免把多项式运算误写成整数运算。
